...
if selected_model == "EC50":
    #loading models
    #classification EC50 - threshold 1000 nM EC50
            ...
            if molecule is not None:
                ...
                with col2:
                    with st.spinner('Calculation in progress'):
                        prediction = classification_model_EC50.predict(descriptors_value_df)
                        st.markdown(f'<div style="{success_style}">Done!</div>', unsafe_allow_html=True)
                        if prediction == 0:
                            prediction_class = "**inactive.**"
                        else:
                            prediction_class = "**active.**"
                        st.write("Class of EC50 for tested molecule towards AhR is ", f'<span style="color: #465e4b;">{prediction_class}</span>', unsafe_allow_html=True)
...
if selected_model == "IC50":
    ...
    if smiles_input_2:
        try:
            molecule = Chem.MolFromSmiles(smiles_input_2)
            if molecule:
                img = Draw.MolToImage(molecule, size=(600, 600))
                with col1:
                    st.image(img, caption='Chemical structure', use_container_width=True)
    ...
    if smiles_input_2:
        try:
            ...
            if molecule is not None:
                ...
                fps_morgan.append(arr_morgan)
                stereo_features.append(analyze_stereochemistry(smiles_input_2))
                df_morgan = pd.DataFrame(fps_morgan, columns=[f'Morgan_{i+1}' for i in range(2048)])
                df_stereo = pd.DataFrame(stereo_features)
                df_combined = pd.concat([df_morgan, df_stereo], axis=1)

                with col2:
                    with st.spinner('Calculation in progress'):
                        prediction = classification_model_IC50.predict(df_combined)
                        st.markdown(f'<div style="{success_style}">Done!</div>', unsafe_allow_html=True)
                        if prediction == 0:
                            prediction_class = "**inactive.**"
                        else:
                            prediction_class = "**active.**"
                        st.write("Class of IC50 for tested molecule towards AhR is ", f'<span style="color: #465e4b;">{prediction_class}</span>', unsafe_allow_html=True)
                        st.write("*The model is based on Morgan fingerprints. Impossible to provide applicability domain.*")
                        st.button("Clear SMILES", on_click=clear_text)
                    def normalize_data(data, max_values):
                        return [x / max_val for x, max_val in zip(data, max_values)]
                    def plot_radar_chart_one_condition(data1, data3, labels, max_values, label, title):
                    ...
                    st.write(' ')
                with st.expander("**See druglikeness and lead-like properties of a tested molecule**"):
                    rotatable_bonds = Calculator(descriptors.RotatableBond.RotatableBondsCount())(Chem.MolFromSmiles(smiles_input_2))[descriptors.RotatableBond.RotatableBondsCount()]
                    logP = Calculator(descriptors.SLogP.SLogP())(Chem.MolFromSmiles(smiles_input_2))[descriptors.SLogP.SLogP()]
                    mw = Calculator(descriptors.Weight.Weight())(Chem.MolFromSmiles(smiles_input_2))[descriptors.Weight.Weight()]
                    hdonors = Calculator(descriptors.HydrogenBond.HBondDonor())(Chem.MolFromSmiles(smiles_input_2))[descriptors.HydrogenBond.HBondDonor()]
                    hacceptors = Calculator(descriptors.HydrogenBond.HBondAcceptor())(Chem.MolFromSmiles(smiles_input_2))[descriptors.HydrogenBond.HBondAcceptor()]
                    labels_RO3 = ['Hydrogen donors', 'Hydrogen acceptors', 'Molecular weight', 'logP', 'Rotatable bonds']
                    labels_RO5 = ['Hydrogen donors', 'Hydrogen acceptors', 'Molecular weight', 'logP']
                    data1 = [5, 10, 500, 5]
                    data2 = [3, 3, 300, 3, 3]
